...
/**
 * Nuxt server-middleware — THIN PROXY for /api/course.
 *
 * The course engine (OpenAI calls + business logic) now lives on the Restify
 * backend at server/courseEngine.js, per the coding-team Req 7 ruling (all
...
module.exports = function courseProxy (req, res, next) {
  ...
  let target
  try {
    target = new URL('/api/course', BACKEND)
  } catch (e) {
  ...
  backendReq.on('error', function (err) {
    console.error('[course-proxy] backend error:', err.message)
    if (!res.headersSent) {
      res.writeHead(502, { 'Content-Type': 'application/json' })
      res.end(JSON.stringify({ success: false, error: { code: 'BACKEND_UNAVAILABLE', message: 'Course backend unavailable' }, timestamp: new Date().toISOString() }))
    } else {
      try { res.end() } catch (e) {}
    }
  })
