  ...
  template(v-for="(step, i) in steps")
    ...
    template(v-for="item in step.items")
      ...
      section.spd-page.is-teach(v-if="item.summary || item.prompts.length" :key="'t' + i + item.key")
        ...
        h3.spd-h(v-if="!titlesItself(item)") {{ item.name }}
        ...
        ul.spd-prompts(v-if="item.prompts.length && !promptsRepeatDrawing(item)")
...
import { hasConceptGraphic, conceptTitlesItself, promptsEchoDrawing } from '~/components/strategy/concepts'
