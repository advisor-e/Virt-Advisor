...
article.spd(:style="frameStyle")
  ...
  template(v-for="(step, i) in steps")
    ...
    template(v-for="item in step.items")
      ...
      //- 🔴 ONE SHEET PER TEACHING PAGE, AND A CONCEPT MAY HAVE MORE THAN ONE —
      //- Mike, 2026-09-23. Collaborative Thinking is his Christchurch story AND his
      //- De Bono explanation, two pages that will not fit one sheet at his own type
      //- sizes. This printed the concept once, so the second sheet would have been
      //- absent from the client's plan with nothing saying so.
      //- ⚠ THE HEADING, THE LEAD AND THE PROMPTS PRINT ON THE FIRST SHEET ONLY, or a
      //- client reads the same title and the same questions twice in a row.
      //-
      //- 🔴 AND A PAGE THAT IS ONLY A DRAWING IS NOT A BLANK PAGE. This asked for a
      //- summary or a prompt, which was right while every concept had one — the guard
      //- exists to keep an empty slide out of a client's document. A concept whose
      //- whole teaching IS his drawing has neither, so its page was dropped and the
      ...
      template(v-for="n in sheetsOf(item)")
        section.spd-page.is-teach(v-if="drawn(item) || item.summary || item.prompts.length" :key="'t' + i + item.key + '#' + n")
          ...
          //- 🔴 NO HEADING WHERE THE DRAWING TITLES ITSELF — Mike, 2026-09-23, on seeing
          //- a teaching page. 20 of the 32 printed the concept's name TWICE, once here and
          //- once inside his own drawing a few millimetres below; ten word for word. His
          //- deck page IS the title. `titlesItself` is generated from the artefact by size
          //- and position (`concepts/index.js`), never guessed from the name — a substring
          //- match called Sales Channel Options titled because a chart label read "sales".
          h3.spd-h(v-if="n === 1 && !titlesItself(item)") {{ item.name }}
          ...
          p.spd-lead(v-if="n === 1 && item.summary && !drawn(item)") {{ item.summary }}
          //- 🔴 NOT WHERE THE DRAWING ALREADY ASKS THEM — item 15.12, ruled fixed by Mike
          //- 2026-09-23. Two concepts repeat their own drawing's questions as bullets:
          //- Porter's 5 Forces (92% of the words) and The 8 Profit Levers (74%). A client
          //- read the same five questions twice on one page, and on A4 the page then grew
          //- past the sheet and split mid-list. Generated by word overlap, so a concept
          //- whose prompts are genuinely extra keeps every one of them.
          ul.spd-prompts(v-if="n === 1 && item.prompts.length && !promptsRepeatDrawing(item)")
...
export default {
  ...
  methods: {
    ...
    /**
     * How many A4 sheets this concept's teaching takes.
     *
     * 🔴 NEVER ZERO. A concept with no drawing still prints one page, of Mike's own
     * words — that is what 20 of the 52 did before item 15.7, and dropping it would
     * take the concept out of the client's plan entirely.
     *
     * Collaborative Thinking is the first to return 2 (Mike, 2026-09-23): his
     * Christchurch story and his De Bono explanation are both nearly full pages at
     * his own type sizes, so neither may be shrunk to join the other.
     *
     * @param {{conceptId: string}} item
     * @returns {number}
     */
    ...
    /**
     * Does this concept's drawing open with a title of its own?
     *
     * If it does, the page must not print a second one above it — Mike, 2026-09-23.
     * Read from the artefact at build time; `vertical-integration` is the one drawing
     * with no title of its own, and its page still needs ours.
     *
     * @param {{conceptId: string}} item
     * @returns {boolean}
     */
    ...
    /**
     * Would this concept's prompt bullets repeat what its drawing already asks?
     *
     * Item 15.12. Two do — Porter's 5 Forces and The 8 Profit Levers — and the flag is
     * generated by word overlap rather than listed, so editing a prompt or a drawing
     * moves it without anyone remembering to.
     *
     * @param {{conceptId: string}} item
     * @returns {boolean}
     */
    promptsRepeatDrawing (item) {
      return promptsEchoDrawing(item.conceptId)
    },
