 ...
 *
 * `firm` renders the Firm Dashboard — a full-screen panel, not a message thread — but
 * was missing from the input's exclusion list, so the message box and its send button
 * rendered underneath it: an input with nothing to talk to.
 *
...
describe('VirtualAdvisor — the chat input appears only in conversational modes', () => {
  ...
  it('declares firm as a panel mode — the reported defect', () => {
    expect(declaredPanelModes()).toContain('firm')
  })
