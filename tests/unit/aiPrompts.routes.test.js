...
describe('what a tier is given when it opens the tab', () => {
  test('a firm manager gets the two client-facing documents and NOT the security one', async () => {
    ...
    expect(res._body.prompts.map(p => p.id)).toEqual([CASHFLOW, ECONOMIC, NEXT_STEPS, DEPRECIATION])
  })
  ...
  test('the mentor gets every document', async () => {
    ...
    expect(res._body.prompts.map(p => p.id)).toEqual([CASHFLOW, SECURITY, REVIEW, ECONOMIC, NEXT_STEPS, DEPRECIATION])
  })
