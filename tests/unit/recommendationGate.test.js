...
describe('the corpus — what the real trees emit today', () => {
  ...
  it('delivers 39 branches whole and 8 in part, and withholds 8 entirely', () => {
    ...
    // recording it. Adjust it deliberately; never delete it. The branches still
    // losing text, and the names they wait on, are in design/LOGIC-TABLE-TEMPLATES-NEEDED.md.
    let full = 0
    let partial = 0
    let withheld = 0
    ...
    expect({ full, partial, withheld }).toEqual({ full: 39, partial: 8, withheld: 8 })
  })
})
