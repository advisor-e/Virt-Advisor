...
- ☐ **P3 · SEC/TEST — Extend the ghost-reference validator to `flat_if_then` trees.** `validateLogicTreeReferences` in [`server/utils/logicTrees.js`](../server/utils/logicTrees.js) only scans `nodes[].templates`, so flat trees (templates in `branches[].templates`) are unguarded — that's how `get_positioning`'s ghosts went undetected. Add `branches[].templates` to the scan, and have it tolerate the `Get. ` catalogue prefix (strip-and-retry) so real-but-mis-prefixed assets like `5 Layers Questionnaire` don't false-positive. *Source:* tree-provenance sweep 2026-06-24.
