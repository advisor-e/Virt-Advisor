...
- **Next ACTION in-session:** **resolve the current branch** `feat/tree-contribution-harness` (8 commits unmerged) — which includes the **valuation provenance decision**, because shipped soft-hint code currently rests on **unverified tree data**. → [§Tree fidelity](#tree-fidelity) · [§Dormant trees](#dormant-trees)
...
- <a id="dormant-trees"></a>◐ **P2 · DECISION+BUILD — 28 dormant trees → harvest JUDGMENT into signals.** Direction LOCKED 2026-06-23 (memory `design-logic-trees-guide-not-replace`): trees GUIDE the engine, don't replace it. **Done:** the soft-hint mechanism (whole tie-breaker bucket, one wiring), valuation wired, `governance_too_early` signal (Option A), name-rot disproven (93/93 real) — all in archive. **Remaining:**
  ...
  - *Method:* `tests/unit/treeContributionHarness.test.js` (current-engine vs tree-assisted). *Source:* registry Part 2; memory `design-logic-trees-guide-not-replace`.

- <a id="tree-fidelity"></a>☐ **P2 · DATA/SEC (needs Mike — provenance) — Two trees carry JSON content NOT in their source PDFs.** Found by the 2026-06-23 fidelity sweep. Full results: [`design/TREE-PDF-FIDELITY-SWEEP-2026-06-23.md`](TREE-PDF-FIDELITY-SWEEP-2026-06-23.md). 6 of 8 wired trees are faithful; **2 are not clean:**
  - ⚠️ **`get_positioning`** — 4 branches' logic faithful, but the JSON **adds 5 template names absent from the PDF** (Business Assessment Report, Revenue Model What-if, Agenda & Notes, Dashboard Discussions, Management Reporting Annual Plan). Now surfaced in Learn-mode positioning coaching. **Decision (Mike):** confirm as intended mappings, or strip the `templates` arrays to what the source supports.
  - ⚠️⚠️ **`valuation`** — the PDF is a flat 13-row valuation-*methodology* table naming **no templates**; the JSON's entire **seller/buyer/exit/MBO skeleton + all Sale-Assessment template names are not in the PDF**. **This session's valuation soft-hint (`327f592`) relies on that unsourced routing + mapping.** Code is sound; the *data* is unverified. **Decision (Mike, provenance):** is the seller/buyer structure real firm IP from another source, or invented in the old bulk-import era and needing a rebuild? Until confirmed, treat the valuation soft-hint result as resting on unverified tree data. *Source:* PDF-vs-JSON sweep 2026-06-23.
