...
- ☐ **WIRE — The 3 proprietary frameworks, Phase 2** (the ★ top priority — this is what delivers Principle P1 for the most valuable IP). Phase 1 (extract to JSON) is done; now wire the live code to **read** the JSON:
  ...
  - ◐ Then **surface all three in Firm Manager** so a firm can edit them (EDIT-TARGET).
    - ◐ **Advisory Staircase — backend DONE 2026-06-09.** Validated `GET`/`POST /api/firm-manager/staircase` (JWT-scoped to `req.firmId`, stored via firm overlay → version history + restore; allowed ceilings derived from the base JSON, not hardcoded; dev fallback). **Override now takes real effect:** `caseState.staircaseToCeiling`/`buildCaseState` accept a pre-blended staircase and the orchestrator (`advisor.js`) blends the firm override over the base per request — proven by tests (197 pass; behaviour-preserving when no override). **Remaining: the `FirmManagerHub.vue` editor panel** (needs label/button wording).
