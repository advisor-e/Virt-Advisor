...
**OPEN, NEITHER BLOCKING:** whether the computed minutes should include the conversation with the AI as well as the material (a "15-minute session" that is a 9-minute video plus a discussion is really 20+); and whether an advisor should be able to set the allowance per firm. | `courseEngine.js` · `courseEffort.js` · `courseSliceCopy.js` · `designInterview.js` · `CourseBuilder.vue` · `CourseMessage.vue` | ✅ **DONE + LIVE-PROVEN 2026-08-03** — suite **4,226 green / 247 suites**, lint 0 errors, pushed. **Mike ran it four times in the real app**; passes 1–2 exposed the range defect, 3–4 exposed all five defects above and confirmed the finished build. |
