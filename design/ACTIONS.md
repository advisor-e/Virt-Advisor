...
**BUILT SO FAR of the new model (`9391775`, pure functions, nothing wired):** `planSessions` reproduces the plan Mike approved exactly (11 sessions, 2h 53m, `[9,20,20,20,15,15,12,12,20,15,15]`) and re-slices the same material to 7 at 25–30 and 6 at 50–60; `splitEvenly` divides evenly rather than leaving a stub (60 min at 20 → 3×20; 30 min at 20 → 15+15, never 20+10); `fitOptions` computes the two figures behind the approved question. **STILL TO DO:** wire the fit question into the interview state machine (it must not collide with the existing outline-revision flow); decide what a sliced session is CALLED and what its objectives are (the AI writes these per template today); render sliced sessions on screen; and the three activity labels are **wording Mike has not yet ruled on**. **ARTEFACT:** [`design/COURSE-SESSION-LENGTH-WORDING.md`](COURSE-SESSION-LENGTH-WORDING.md) — the approved outline-card wording, the option rejected, the exact strings, and the one named deviation. Per CLAUDE.md → "Save the Artefact", this entry **LINKS** the file and does not replace it; open it beside the screen before changing any of that copy. | `courseEngine.js` · outline/resource display in `CourseBuilder.vue` · master export | ◐ Measuring SHIPPED + LIVE-PROVEN 2026-08-03 (`f724c76`, range defect fixed `7a119b8`, slicer `9391775`) — suite **4,132 green / 244 suites**, lint 0 errors. **Mike ran it twice in the real app**: pass 1 exposed the range defect, pass 2 confirmed both notices fire. Phase 2 ABANDONED, replaced by [`COURSE-SESSION-PLANNING.md`](COURSE-SESSION-PLANNING.md). **NEXT: wire the fit question, name a sliced session, render it** — activity labels need Mike |
