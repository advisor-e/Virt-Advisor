...
> 2. **Frameworks → Firm Manager** — wiring is done; **Advisory Staircase** is now fully firm-editable (backend + no-code UI tab). Remaining EDIT-TARGET work: repeat the proven pattern for **Growth Fundamentals** + **3 Engagement Types**.
...
- ☐ **WIRE — The 3 proprietary frameworks, Phase 2** (the ★ top priority — this is what delivers Principle P1 for the most valuable IP). Phase 1 (extract to JSON) is done; now wire the live code to **read** the JSON:
  ...
  - ◐ Then **surface all three in Firm Manager** so a firm can edit them (EDIT-TARGET).
    - ✅ **Advisory Staircase — DONE 2026-06-09 (backend + UI).** Backend: validated `GET`/`POST /api/firm-manager/staircase` (JWT-scoped to `req.firmId`, stored via firm overlay → version history + restore; allowed ceilings derived from the base JSON, not hardcoded; dev fallback). Override takes real effect: `caseState.staircaseToCeiling`/`buildCaseState` accept a pre-blended staircase and the orchestrator (`advisor.js`) blends the firm override over the base per request (197 tests; behaviour-preserving when no override). UI: structured no-code "Advisory Staircase" tab in `FirmManagerHub.vue` (per-step name/description/ceiling editor, default-ceiling select, save/reset/version-history+restore) — entry point to the hub is the **"AI Admin"** button (wording confirmed by Mike).
    - ☐ **Growth Fundamentals** + ☐ **3 Engagement Types** — repeat the now-proven pattern. NOTE for Engagement Types: data lives in two places after Option C (type definitions + `defaultEngagement` in `engagement-types.json`, and the per-domain `engagementType` in `domains.json`) — pin down which surface a firm edits before building.
    *Source:* registry Part 2 + Outstanding item 6; memory `design-engagement-types-extraction`, `design-education-gates-ascent`.
