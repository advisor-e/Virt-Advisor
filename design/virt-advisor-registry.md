...
| **Growth Fundamentals Framework** (`growth-fundamentals.json`, `growth-curve-reveal-reference.json`) | **proprietary framework** | Q6 Growth Curve selector | Stage 1 (Lens 2) · Growth Framework tier · Stage 5 | scale → fee sensitivity + complexity ceiling | ✓ | ⚠ JSON master live + drives the Stage-5 AI narrative (`growth.js` reads it); but the selector (`VirtualAdvisor.vue growthStages`) + detector (`growth.js GROWTH_STAGE_NAMES`) are **hard-coded duplicates** → wire both to the JSON (3-copy list; one drifted line fixed 2026-06-08) |
...
- **How primary issue is derived (today):** the advisor **selects** it from a `[PRIMARY_ISSUE_SELECTOR:domain]` card populated from `primary-issues.json[domain]` (skipped for context domains conflict/eoy/due-diligence, which have none). Stage 2 itself does **not** fall back to the trees — primary-issue derivation is advisor selection only. The trees' actual live consumption is elsewhere: the 14 `mode: learn` trees feed Learn-mode + deep-dive coaching content, and the 28 diagnostic trees are a **rarely-hit Stage-4 fallback** (used only when the template resolver returns zero candidates — see Part 2 + Stage 4). So "dormant" here means *fallback-only*, not disconnected.
