...
- **24 dormant** (diagnostic, no `mode`) — **built but consumed by NO live decision path** (the Client flow uses advisor primary-issue selection instead): `quickfire`, `client_sales`, `cashflow`, `governance`, `client_planning`, `staff_performance`, `frameworks_find`, `systems`, `risk_management`, `valuation`, `succession`, `profitability_feasibility`, `due_diligence`, `get_sales_tracker`, `get_marketing`, `get_positioning`, `get_team_problem`, `get_pricing_proposals`, `stock_purchasing`, `raising_capital`, `get_seminar`, `financial_systems_review`, `three_pill_fin_mgt`, `cash_tactics`.

**The four firm-facing tables are no longer among them (2026-08-25).** `fm_coach_culture`, `org_ca_firm_strategy`, `org_firm_board_pack` and `org_leadership` carry `mode: 'learn'` and `section: 'get-organised'`, which is where the 2026-06-23 triage placed them: the firm/learning surface, not the client engine. They are reachable in Learn mode and barred from the client soft hint and the client deep dive. Guarded by [`tests/unit/firmFacingTrees.test.js`](../tests/unit/firmFacingTrees.test.js). **24 dormant, not 28.**
...
- **Assets:** `primary-issues.json` (the canonical per-domain list — ⚠ **no code reads it**, see below), `logic_trees.json` (42 trees, currently dormant fallback).
...
- ⚠ **The selector's CODE is gone too, as of 2026-08-14** — card, state, methods, styles and a duplicate of the list that had been sitting in `VirtualAdvisor.vue` since before the retirement. Removing it from intake in June left roughly 100 lines of unreachable screen code behind, and the list existed twice with nothing keeping the copies honest (they were identical when checked, which was luck). **The marker strip was deliberately KEPT on both reply paths** — nothing emits `[PRIMARY_ISSUE_SELECTOR:…]`, but a model is not a compiler, and an advisor must never read the raw marker in a reply. Pinned by `tests/unit/retiredPrimaryIssueSelector.test.js`. **`data/primary-issues.json` was kept** — it is authored Workshop 1 content, and deleting content is not the same act as deleting dead code.
...
- **"None of these fit" escape — now carried by the contradiction detector above, not a sentinel.** The `__none_of_these__` sentinel and its `noneOfTheseApply` handler were **deleted on 2026-08-15** and neither exists in the code today. The escape itself is undiminished: the advisor says so in their own words, `_CONTRADICTION_PATTERN` (line above) catches *"none of these"* / *"that's not the issue"* / *"wrong area"*, and the pipeline pauses and asks. **The design principle stands — a constrained selector must always let the advisor reject the read** — only the mechanism changed, from a menu option to plain language.
