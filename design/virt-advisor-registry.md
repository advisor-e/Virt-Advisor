...
**P1 — Auditable AND editable (with proprietary frameworks protected).** Every decision the system makes must be (a) traceable to a reason, and (b) changeable by a firm manager in the Firm Manager page (table/flowchart/slider) — never requiring a code change. This is the whole point of the system. **Exception — protected IP:** certain proprietary frameworks are deliberately **platform-locked and NOT firm-editable**, to protect the IP they represent. They stay wired to their single-source JSON (so they drive the engine and the AI without code changes), but a firm cannot rewrite them. **Growth Fundamentals is the standing example** (Mike's decision, 2026-06-11). Whether a framework is firm-editable is a per-framework decision and is never assumed — it requires explicit sign-off.
...
| **Advisory Staircase — 5 Advisor-e Steps** (source: `Logic Tables/5 Advisor-e Steps.logic.pdf` + `Domain Support/_5 Advisor-e Steps.supt.pdf`; live steps hard-coded in `VirtualAdvisor.vue`) | **proprietary framework** | Q7 Staircase selector | Stage 1 (Lens 3) · Stage 3 | relationship depth → complexity ceiling + comprehensiveness | ✓ | ⚠ extracted → `advisory-staircase.json` (Phase 1, inert, 2026-06-08); awaiting wiring (move steps + ceiling out of `caseState.js` / `VirtualAdvisor.vue`). Carries `education-gates-ascent` designIntent |
| **3 Engagement Types** (source: `Logic Tables/3 Engagement Types.logic.pdf` + `Domain Support/3 Engagement Types.supt.pdf` + `The 3 Engagement Types.pdf`; live in `client.txt` + `strategyResolver.js`) | **proprietary framework** | derived (not asked) | Stage 3 · Axis 2 · Stage 5 | engagement type + delivery style | ✓ | ⚠ extracted → `engagement-types.json` (Phase 1, inert, 2026-06-08); awaiting wiring — incl. **Option C**: move `DOMAIN_NATURAL_ENGAGEMENT` to a per-domain field in `domains.json` |
...
**Reading it:** Advisory Distinctions is the only block both live AND editable in Firm Manager. The proprietary frameworks were the top migration priority for **wiring to single-source JSON** (now done for all three). **Firm-editability, however, is per-framework and not automatic** — Growth Fundamentals is explicitly platform-locked (Mike, 2026-06-11), so Principle P1's "editable by a firm" applies only to the blocks a firm is *meant* to own. All three now have JSON: **Growth Fundamentals** (`growth-fundamentals.json`, already drives the AI narrative), **Advisory Staircase** (`advisory-staircase.json`, extracted 2026-06-08) and **3 Engagement Types** (`engagement-types.json`, extracted 2026-06-08). **Phase 1 (extract to editable JSON) is done; Phase 2 wiring — pointing the live selectors/logic at the JSON — was completed 2026-06-09 for all three (the 3-copy duplication is gone).** **Surfacing the frameworks in Firm Manager for firm editing is NOT a blanket goal:** **Growth Fundamentals is platform-locked — NOT firm-editable (Mike's decision 2026-06-11)**, an explicit exception to Principle P1; the **Advisory Staircase** edit-target is done; **3 Engagement Types** needs explicit sign-off before any build. See Outstanding item 6. *(The "surface all three so a firm can edit them" goal was an assumption that accreted from generalising the staircase — never a decision.)*
