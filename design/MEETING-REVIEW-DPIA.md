...
| **IPP6** | Access — an individual may ask for their information | ⚠ **NOT BUILT.** There is no route by which a client's request for a copy of their transcript is served. It can be answered manually, and the data is findable, but nothing in the software supports it. **This is ours.** See §9 |
| **IPP7** | Correction — and to attach a statement if you refuse | ⚠ **Partly.** The advisor can dispute any observation and the dispute stays in the record, which serves IPP7 for the *advisor*. **For the client there is no mechanism**, and a transcript is a record of what was said rather than a claim of fact — but a client's correction statement still has to be able to attach to it. **This is ours.** See §9 |
...
4. **How it handles a withdrawal of consent that arrives later** — by email, a week after the
   ...
   arrives afterwards.
5. **How it answers a client asking for a copy of their transcript, or its deletion.** Until the
   §6 IPP6/IPP7 gap is closed on our side, this is a manual process a firm must have.
...
| **B** | **IPP6 and IPP7** — a client's access to, and correction of, what was recorded about them | 🔴 **Ours** | ⚠ **Not built and not decided.** This is item 7 of the Brief's §4 and it has no answer anywhere. In a UK/EU market it is a hard right with a one-month deadline (§7) |
