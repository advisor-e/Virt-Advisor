...
| Audio, for transcription | **OpenAI**, already this application's contracted sub-processor | ⚠ **Asked twice, never answered — §6 IPP12 and [`OPENAI-AUDIO-TERMS-EMAIL.md`](OPENAI-AUDIO-TERMS-EMAIL.md) §5.** Region unnamed |
...
| **IPP12** | 🔴 **Disclosure outside New Zealand — satisfy yourself the recipient has comparable safeguards** | ⚠ **EVIDENCED BY PUBLISHED TERMS AND DOCUMENTED EFFORT, NOT BY CONFIRMATION — Mike's ruling, 2026-09-18.** Both channels OpenAI nominates were asked and neither answered; the replies are verbatim in [`OPENAI-AUDIO-TERMS-EMAIL.md`](OPENAI-AUDIO-TERMS-EMAIL.md) §5. The Act requires the *disclosing agency* to satisfy itself, and what it is satisfied **by** is now on the record: the published DPA and data-controls pages, plus two unanswered approaches. 🔴 **Processing regions were never named, and Support cautioned against assuming the general API terms extend to audio** — so this is a reasoned position a regulator can read and weigh, never a confirmed one. **A lawyer's review per market (Brief §4 item 2) is where that position gets tested; it no longer waits on anything** |
...
| | What | Whose | State |
|---|---|---|---|
| **A** | **IPP12** — the transcription provider's processing regions, retention, training position and human review, confirmed in writing | **Ours to obtain** | ⚠ **ASKED AND NOT ANSWERED. Closed 2026-09-18 on Mike's ruling, NOT on a confirmation** — both replies verbatim in [`OPENAI-AUDIO-TERMS-EMAIL.md`](OPENAI-AUDIO-TERMS-EMAIL.md) §5. **What is relied on:** OpenAI's published DPA and data-controls pages, plus two documented attempts through both channels they nominate. 🔴 **Support explicitly declined to confirm the general API terms extend to audio.** State that basis whenever this is questioned; never describe it as confirmed |
