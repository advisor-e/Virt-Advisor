...
something"*. The rail is now the way through the session rather than a picture of it — real
buttons, so a keyboard reaches them, and the last four dimmed until a session exists, because they
save into one and would otherwise draw an empty meeting and discard what was typed. **Nothing is
lost by jumping**: every box saves as it is typed and `seedSteps` only ever fills an empty list, so
a return visit to Scope never disturbs the arrangement already built. 🔴 **LEAVING SCOPE IS NOT
JUST A SCREEN CHANGE** — it fetches any newly ticked concept's capture table and saves the scope
first; without that the concept is silently absent from everything downstream (fault 6 below).
**The primary button also stopped moving**: it sat 61px lower on Scope than elsewhere and wandered
166px sideways across the stages, because `.sp-top-actions` had no rule at all. Its right edge is
now the same on all five. **And it reads *Build the session*** — his wording — so it names the
stage it opens rather than implying the meeting is starting.
...
#### 🔴 What BUILDING the Org Chart found — six faults, not one caught by 12,600 tests (2026-09-21)

**Every one was found by opening the screen and clicking, or by a machine comparison, with the
whole suite green throughout.** This is the same lesson as the box above, one form later, and it
is why the run-it-and-look step is not optional. **Three were in the screen, one was in the
drawing, and two were older faults the work walked into.**

**1 — a removed person came back in the "empty" row.** Remove the LAST role, press *Add a role*,
and the new row arrived holding **Personal Assistant** and their reporting line. A removed role's
boxes are deliberately left in the store — that is what makes a DELETE route unnecessary — so an
id taken from the roster alone falls back the moment the highest one goes. `nextRoleId` now reads
the entries as well as the roster. ⚠ **The unit test that was meant to catch this passed**: it
removed a role from the MIDDLE of the list, where the highest id survives and the fault cannot
appear. The test now removes the last one, and the one after that empties the list entirely.

**2 — Decision 11's timeline had been refusing every workbook box for four days, and NOT only
this form.** `postTimeline` checked only the built frameworks' whitelist; `putEntries` checks that
one **and** the concepts' own tables. So when those tables arrived on 2026-09-17 the SAVE learnt
about them and the TIMELINE did not: every box on the 16 concepts read from Mike's workbooks saved
perfectly and had its timeline entry refused with a 400. **Nothing on any screen said so**, because
the page swallows a timeline failure on purpose — an error banner mid-sentence costs the advisor
more than the gap does. Proved against the running backend with a **Porter's** box, which has
nothing to do with the Org Chart, then fixed on Mike's ruling the same day. ⚠ **THE TWO WHITELISTS
MUST BE CHANGED TOGETHER.** Three tests now hold it, including one proving a made-up box is still
refused, so widening the guard has not turned it into no guard.

**3 — the redrawn drawing put five boxes 96px out of place, and the BUILD was right.** Its first
version was drawn at one box size and re-scaled to another, and the rescale rounded each box to a
whole column — **a parent centred over its children legitimately sits on a HALF column**, so CEO,
Board, Shareholders, COO and Head HR were all drawn to the right of the children they sit over. It
looked entirely plausible. **Only the machine comparison in `tests/unit/orgChart.test.js` found
it**, which is the argument for that test: nobody checks a chart by eye and sees 96px. The rescale
was removed rather than patched, so the drawing is now `layout()`'s own output.

**4 — renaming a role silently deleted the person in it.** `renameCascade` rebuilt each role from
three of its four fields. Caught by a new test, not by the screen; the save path happened not to
hit it, so it would have surfaced later as somebody vanishing for no reason.

**5 — the stage rail had been lying about progress since Build session was added.** Its order read
`scope, run, objectives, plan` — **`steps` was missing**, so `indexOf` returned -1 and on Scope,
the first screen an advisor ever sees, segment 2 rendered as DONE (`-1 < 0`), while on Build
session segment 1 rendered as untouched. **A green segment reads as progress, not as a mistake.**
There is now one list, `railStages`, that the rail and the progress rule both read.

**6 — going back to Scope silently dropped a newly ticked concept, which is the exact journey the
clickable rail was built for.** A concept's capture table was fetched only when a session
*started*, and `conceptVisits` skips a concept with nothing in `captures` — so one ticked on a
RETURN visit never appeared in Build session, the meeting or the client's plan. The advisor ticks
...
their boxes on the page. Before anything built from it ships, **open the drawing, put it beside the build,
