...
**Five stages of the eight remain**, and the one immediately ahead — **the step builder, stage 3b**
— is **specified and not built**: Mike ruled the gating question and then all five decisions on its
drawing on 2026-09-20, every one as recommended.
...
| | Stage | The request it serves | What it buys |
|---|---|---|---|
| **3b** | 🔴 **THE STEP BUILDER — SPECIFIED 2026-09-20, NOT BUILT.** A new stage **2** in the rail, between Scope and Run, taking it from four stages to five: the advisor names each step and drags ticked concepts into it. **Drawn and approved:** [`../mockups/strategy-step-builder.html`](../mockups/strategy-step-builder.html), **all five decisions ruled by Mike 2026-09-20, every one as recommended** — opens with everything unplaced · may run with concepts unplaced, the button saying *"Run 11 of 14"* · the closing block becomes an ordinary renameable step · names are **free text with nothing offered**. ⚠ **He places CONCEPTS, not pages:** a concept bringing two teaching slides carries both; only a two-part fill-in table splits, into the two cards that already exist. **It is the INPUT only — [`StrategyPlanDocument.vue`](../../components/strategy/StrategyPlanDocument.vue) already takes a list of steps, prints one agenda line each and already handles a step with nothing in it.** | 🔴 **Mike's own words, 2026-09-20:** *"I choose the number of stages, I choose the name i put on that stage, I choose the number of concepts/pages I put into each stage — correct? if not, thats what i want"*, and on the label, *"steps is fine"*. Carries out his ruling of the same day (item 15.9): **the advisor names the steps himself, and a step holding nothing still prints on the agenda.** | **Measured against Pivot before a line was drawn.** The app makes **one** step where Pivot needs **five** — and having no name of its own it borrows the wizard's, so a client's agenda today reads *"Run the frameworks — 14 concepts"* and *"Objectives and actions — 2 concepts"*: **our internal screens, printed on their plan.** Also **0 of 14** concepts placed by the advisor, an empty step **impossible**, and Porter's two visits stuck in one step where Pivot runs them an hour apart. The journey end to end is [`strategy-planner-walkthrough.html`](../mockups/strategy-planner-walkthrough.html). |
