...
| The secret is enforced, not assumed | `server/utils/outcomePoolBootCheck.js` — the server **refuses to start** when any firm shares and the secret is unset, naming the count and no firm id; switching sharing on is refused with `503 POOL_UNAVAILABLE` and writes nothing. Without this the pool silently drops every contribution while the screen still reads "Sharing since …". A store it cannot read never stops the boot |
