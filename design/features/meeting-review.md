...
| The meeting TYPES themselves | `server/utils/meetingTypes.js`, `server/routes/meetingTypes.js`, `components/firm/FirmMeetingTypes.vue` | ✅ **BUILT (slices 1–2, 2026-09-02)** — a type carries its own `name` and an optional `treeId`, and the mentor creates, renames, reorders and switches them off. Four config keys (declines · overrides · own · **order**), resolving through all four manager tiers. ⚠ **Slice 3** opens the controls to the other three tiers — one computed property — and **slice 4** adds the advisor and business-entity levels, which need the new storage shape. [`../MEETING-TYPES-CASCADE.md`](../MEETING-TYPES-CASCADE.md) §7 |
