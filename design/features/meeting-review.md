...
| Manager routes | `server/routes/meetingObservations.js` | ✅ **BUILT** — 9 routes, all scoped to `req.firmId`, **plus 5 advisor-level routes and the manager's set-aside view** (2026-09-08). The advisor ones key every write to `req.advisorId` from the verified token and never from a body |
| What advisors have set aside | `setAsideSummary`, `GET /api/firm-manager/meeting-observations/set-aside`, a panel on `FirmMeetingObservations.vue` | ✅ **BUILT 2026-09-08 — ORDERED BY MIKE** in the same breath as permitting the thing it shows: *"yes but fix the issue - build it so the manager can see"*. Counts and **names** the advisors who took each point off their own list; lists every point, including untouched ones. **FIRM TIER ONLY** — the decisions live on each firm's row, so a higher scope has none beneath it; 403 above. **A mirror, not a control**: no route puts a point back on an advisor's list, because P14 runs downward only. 🔴 **NO DENOMINATOR — this app holds no advisors table** (`config/db-schema.sql`, four times), so a firm's headcount is unknowable and "4 of 12" could not be built; Mike's own line says what the number is instead |
...
| The meeting TYPES themselves | `server/utils/meetingTypes.js`, `server/routes/meetingTypes.js`, `components/firm/FirmMeetingTypes.vue` | ✅ **BUILT (slices 1–3; 1–2 on 2026-09-02, 3 on 2026-09-08)** — a type carries its own `name` and an optional `treeId`, and **every manager tier** creates, renames, reorders and switches them off. Four config keys (declines · overrides · own · **order**), resolving through all four tiers. Slice 3 was the one computed property slice 2 predicted; the `visible` gate it removed **was never a permission check and must not become one** — P14 is enforced by every route being scoped to `req.firmId`. ⚠ **Slice 4** adds the advisor and business-entity levels, which need the new storage shape *and* the pre-set naming each point's source (D1/D2). [`../MEETING-TYPES-CASCADE.md`](../MEETING-TYPES-CASCADE.md) §7 |
| The advisor's pre-set | `components/MeetingPreset.vue`, `pages/meeting-preset.vue` | ✅ **BUILT, AND EDITABLE SINCE 2026-09-08** — the advisor's own level, from [`../mockups/meeting-preset-advisor-level.html`](../mockups/meeting-preset-advisor-level.html), all six of its questions ruled that day. Every point names the tier it came from (three labels; the middle tiers collapse into *"From your firm"*), an advisor may set an inherited point aside **for themselves only**, and may add points of their own **with hint words** — the one ruling that REVERSED the recommendation. Storage: two keys on the firm's own row, `server/utils/meetingObservationsAdvisor.js`. 🔴 **AND IT SHAPES THE REPORT, WHICH IT DID NOT AT FIRST** — `presetFor` in `server/routes/meetingReview.js` applies the advisor's layer through the SAME `applyAdvisorLayer` the screen calls, never a second copy. Until that was fixed the same day, a point the advisor had set aside was still assessed and one they had written themselves never was, while their screen showed the list saved and correct. An over-long hint phrase is **refused**, not trimmed away behind a `200`. ✅ **THE BUSINESS-ENTITY LEVEL IS BUILT TOO (2026-09-10)** — a second picker, *"Who is this meeting with?"*, from the same register the recorder reads; with a client picked the list is that relationship's, resolved firm → advisor's own layer → the client's on the backend (`server/utils/meetingObservationsEntity.js`, `server/routes/meetingObservationsEntity.js`), and `presetFor` applies the same layer to the report. From [`../mockups/meeting-preset-client-level.html`](../mockups/meeting-preset-client-level.html), all five questions ruled by Mike that day: **one shared list per client** that any advisor in the firm may edit, **every entry named**, able only to set aside or add — never to put back what an advisor set aside for themselves — and 🔴 **no manager screen**: *"it's the PARTNER or firm manager that owns the client data base - NOT the advisor. WHY are we building two levels of this??"* A firm manager opens this screen, or Advisor-e's own view-as-advisor |
