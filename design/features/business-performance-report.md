...
| Planned, none exists yet | the monthly and five-year views (stage 5); *Stock against the accounts* now the reader exists (stage 4, slice 2), with its own model and golden test, and the drop zone on step 3; The three accounts-only pages are BUILT (slice 2, below); the two threshold rows are BUILT and Mike's four figures are in (2026-09-08: current ratio 1.5 / 1.0, debt to equity 1.0 / 2.0), so the score counts eight |
