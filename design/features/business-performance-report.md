...
| Planned, none exists yet | a benchmarker data file + mentor-hub upload + industry finder (stage 3); an inventory-export reader under `server/report/intake/` (stage 4); the monthly and five-year views (stage 5); *Stock against the accounts* after the reader (stage 4), with its own model and golden test; The three accounts-only pages are BUILT (slice 2, below); the two threshold rows are BUILT and Mike's four figures are in (2026-09-08: current ratio 1.5 / 1.0, debt to equity 1.0 / 2.0), so the score counts eight |
