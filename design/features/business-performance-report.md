...
| Planned, none exists yet | a benchmarker data file + mentor-hub upload + industry finder (stage 3); an inventory-export reader under `server/report/intake/` (stage 4); the monthly and five-year views (stage 5); the four optional pages drawn on 2026-09-08 — the three accounts-only pages first, then *Stock against the accounts* after the reader — each with its own backend model and golden test; the two threshold rows are BUILT (2026-09-08) and ship empty — the score counts eight the day Mike types the four figures on the mentor's trend-thresholds page |
