...
> **Mike's to action when the search content can be updated.** Fourteen branches across three
...
**A sentence dies if ANY name in it is missing.** Three branches name two documents and need both.

⚠ **`Team AI` may not be a document at all** — it reads like a concept inside other material. If
it is, the fix is to reword that sentence and no page is needed.

---

## The 18 names, spelled as the app looks them up
...
| Client has a history of repeating mistakes, ignoring negative data, or making gut-feel choices | `Decision Workpaper` | Use Decision Workpaper to structure future high-stakes decisions. | Use Quality Decisions to introduce the psyche errors concepts. |
| Client is making assumptions or taking action based on sudden spikes or drops in business metrics | `Interpreting Data Correctly` | Use Interpreting Data Correctly framework. | Apply Demings Volatility to classify the cause type before recommending any action. |
...
## Leadership & Partner Development — 6 branches
...
Today it reports **39 whole / 8 partial / 8 withheld**. The counter is also pinned in
`tests/unit/recommendationGate.test.js`, which fails deliberately when the numbers move — adjust
it to the new figures. When a name resolves, rename it in the branch's recommendation AND put the
published title in that node's `templates[]`: the ranking engine reads only the array, never the
prose.
