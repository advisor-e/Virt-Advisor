# OpenAI "Data controls in the OpenAI platform" — verbatim, as read 2026-09-24

> **Source:** https://platform.openai.com/docs/guides/your-data, pasted by Mike Barnes on
> 2026-09-24. **This is the page clause 7 and the definitions of the ZDR amendment point to**
> ([`ZDR-AMENDMENT-2026-09-24.md`](ZDR-AMENDMENT-2026-09-24.md)), so it forms part of what the
> contract means.
> **It is a live web page and OpenAI changes it.** This copy records what it said on this
> date. Before relying on a detail for a new feature, re-read the live page and add a new dated
> copy beside this one if it has moved. **Never edit this copy.** Only the table was turned into
> markdown; the region listings at the end are kept exactly as the page rendered them.
>
...
- Abuse monitoring logs: Logs generated from your use of the platform, necessary for OpenAI to enforce our Usage Policies and agreements and mitigate harmful uses of AI.
...
- When Zero Data Retention is enabled for an organization, the store parameter will always be treated as false, even if the request attempts to set the value to true.
- Background mode stores response data to disk for roughly 10 minutes to enable polling. For projects using Modified Abuse Monitoring, including enhanced Modified Abuse Monitoring, foreground requests follow standard retention when store is omitted or set to true. Background responses follow the standard retention period only when the request explicitly sets store=true. If store is omitted or set to false for a background request, the response is deleted after the temporary polling period.
- Audio outputs application state is stored for 1 hour to enable multi-turn conversations.
- See image and file inputs.
...
Contact our sales team to see if you're eligible for using data residency controls. Data residency endpoints are charged a 10% uplift for models released on or after March 5, 2026, that are eligible for data residency.
...
OpenAI uses sub-processors to provide its services. For requests sent to us.api.openai.com or eu.api.openai.com, OpenAI uses Cloudflare Regional Services so that TLS termination and HTTPS decryption occur within the selected processing region.
...
#### Support by region (as the page rendered it)
...
Storage Yes  Processing Yes
/v1/audio/transcriptions, /v1/audio/translations, /v1/audio/speech  Storage Processing
/v1/batches  Storage Processing
...
Requires MAM or ZDR**
/v1/audio/transcriptions, /v1/audio/translations, /v1/audio/speech  Storage Processing
/v1/batches  Storage Processing  gpt-6-astra  gpt-5.5-pro-2026-04-23  +30 snapshots
/v1/chat/completions  Storage Processing
Show 22 more services
...
au.api.openai.com
Text, Audio, Voice, Image*
Storage Yes  Processing No
Requires MAM or ZDR
/v1/audio/transcriptions, /v1/audio/translations, /v1/audio/speech  Storage
/v1/batches  Storage
/v1/chat/completions  Storage
Show 17 more services

Canada — ca.api.openai.com — Text, Audio, Voice, Image* — Storage Yes, Processing No — Requires MAM or ZDR — (same services, Storage only) — Show 17 more services
Japan — jp.api.openai.com — Text, Audio, Voice, Image* — Storage Yes, Processing No — Requires MAM or ZDR — (same services, Storage only) — Show 17 more services
India — in.api.openai.com — Text, Audio, Voice, Image* — Storage Yes, Processing No — Requires MAM or ZDR — (same services, Storage only) — Show 17 more services
Singapore — sg.api.openai.com — Text, Audio, Voice, Image* — Storage Yes, Processing No — Requires MAM or ZDR — (same services, Storage only) — Show 17 more services
South Korea — kr.api.openai.com — Text, Audio, Voice, Image* — Storage Yes, Processing No — Requires MAM or ZDR — (same services, Storage only) — Show 17 more services
United Kingdom — gb.api.openai.com — Text, Audio, Voice, Image* — Storage Yes, Processing No — Requires MAM or ZDR — (same services, Storage only) — Show 17 more services
...
/v1/audio/transcriptions, /v1/audio/translations, /v1/audio/speech  Storage
/v1/batches  Storage
/v1/chat/completions  Storage Processing  gpt-5.6-luna  gpt-5.5-2026-04-23  +1 snapshot
Show 17 more services
...
(Canada through the UK were rendered as full blocks identical in shape to Australia's; they are
condensed to one line each here with no change to any value.)

#### API Endpoint, tool and model support (as the page rendered it)

```
25 supported services
Endpoint or feature	Service	Supported models	Notes
/v1/audio/transcriptions, /v1/audio/translations, /v1/audio/speech	Audio	tts-1, whisper-1, gpt-4o-tts, + 3 model snapshots	—
/v1/batches	Batches	gpt-6-astra, gpt-6-sol, gpt-6-luna, + 31 model snapshots	For GPT-6 Sol and Luna, EU data residency is available only with Standard processing.
/v1/chat/completions	Chat Completions	gpt-6-astra, gpt-6-sol, gpt-6-luna, + 25 model snapshots	For GPT-6 Sol and Luna, EU data residency is available only with Standard processing.
/v1/embeddings	Embeddings	text-embedding-3-small, text-embedding-3-large, text-embedding-ada-002	—
/v1/evals	Evals	Supported	—
/v1/files	Files	Supported	—
/v1/fine_tuning/jobs	Fine-tuning	gpt-4o-2024-08-06, gpt-4o-mini-2024-07-18, gpt-4.1-2025-04-14, + 1 model snapshot	—
/v1/images/edits	Images	gpt-image-2, gpt-image-1, gpt-image-1.5, + 1 model snapshot	—
/v1/images/generations	Images	gpt-image-2, gpt-image-1, gpt-image-1.5, + 1 model snapshot	—
/v1/moderations	Moderation	omni-moderation-latest	—
/v1/live/sessions	GPT-Live	gpt-live-1	—
/v1/realtime	Realtime	gpt-realtime, gpt-realtime-1.5, gpt-realtime-mini, + 3 model snapshots	—
/v1/realtime/transcription_sessions	Realtime	gpt-realtime-whisper, gpt-live-transcribe, gpt-transcribe	—
/v1/realtime/translations	Realtime	gpt-realtime-translate	—
/v1/responses	Responses	gpt-6-astra, gpt-6-sol, gpt-6-luna, + 31 model snapshots	For GPT-6 Sol and Luna, EU data residency is available only with Standard processing.
...
Supported input modalities	Other	Text, Image, Audio/Voice	—
```
