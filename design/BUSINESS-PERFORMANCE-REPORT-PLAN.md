...
| B2 | **Add "Back to navigation" on every model page** — after opening a model from the launcher there's no way back to the Model Library | Bug | P2 | ◐ | Owner-reported 2026-07-09 (EOD). **Mockups done 2026-07-13** — "← Back to Model Library" link (wording owner-approved) added above the eyebrow in all 3 model mockups, targeting `model-library-launcher.html`. **App half still open:** the 3 report components (`BusinessPerformanceReport`, `DebtorDragReport`, `MarginBreakevenReport`) have no back link because **the app has no model-library route** — the launcher exists only as a mockup. Blocked on the owner deciding the destination (build a `/model-library` Nuxt page vs. return to Advisory.com). When built: label must go through `$t()` per the Stack Constitution (no hardcoded English in templates); use Nuxt 2's `<nuxt-link>` |
