...
| T26 | **Model Library — group the grid BY CLASS, with category filtering underneath** | Build | P2 | ☐ | Owner-requested 2026-07-13. Today the library is one flat grid of cards with a class badge on each. Instead it should **break into the three sections from T25 — Education / Decision tool / Client report — as the primary split**, so an advisor sees "teaching aids" and "client reports" as distinct shelves rather than a mixed pile. Within that, keep the existing filtering: **All**, then the class filter, then the category (Cash Flow / Profitability / …) breakdown beneath. Depends on T25 (done) — the `modelClass` field and labels already exist in [`utils/reportModelCatalogue.js`](../utils/reportModelCatalogue.js), so this is presentation work in [`components/ModelLibrary.vue`](../components/ModelLibrary.vue) |
