...
| 2026-07-16 | **New dependency RULED (owner): `xlsx` (SheetJS) pinned `0.18.5`** for reading Excel exports on the Restify backend — pure JS, no native components, runs on the locked Node 14.15. Flagged and approved per the stack-governance rule before install (install happens at Stage B). CSV parsing needs no library. |
