...
>
> **Status:** planning started. **Last updated:** 2026-07-16.
>
...
| T2 | **Port each Excel model's calculation logic into a live backend calc engine** (inputs → outputs; editable variables → recalculated figures) — not just reference | Design | P1 | ◐ | Backend-only per stack; enables live editing (T17). **First model DONE (deconstruct + validate): `GE.3c.Working Capital Cycle model` — all 28 calc cells reproduced exactly.** **First REPORT-class model chosen + proven 2026-07-16: `CM. Quick Position.` — deconstructed (3 sheets: diagnosis, Expenses Review, empty NZ-info), NZ COVID wage-subsidy block DROPPED by owner ruling (incl. its 2 dependent cells), all 32 in-scope calc cells reproduced exactly (golden set for the engine's tests). No other maths adaptation needed — the rest ports faithfully.** Others to follow, by demand |
| T3 | Define the report **data model** — every figure the report needs, and where it comes from | Design | P1 | ◐ | **RE-SCOPED by T25: applies to the 9 Report-class models ONLY — none of which is built yet. The 3 built models are Education and need no data model at all.** **DRAFT 2026-07-13 → [`design/REPORT-DATA-MODEL.md`](REPORT-DATA-MODEL.md).** Full figure inventory for the 3 built models is done and is **fact** (read from the model code): 20 named inputs = 41 numbers. **Key finding: only ~⅓ can come from an accounting export** — unit price, unit cost, mark-up, discount, lead times etc. are operational facts no accounting system holds (WCC alone: 6 of 12 inputs). Hence the **hybrid intake** decision (owner, 2026-07-13): file seeds what it can, advisor completes the rest, every figure tagged *from file* / *entered*. **VERIFIED 2026-07-13** against 4 real Xero exports (Demo Company NZ, supplied by owner): **3 assumptions refuted** — prior-year revenue and monthly sales are NOT in the standard P&L export, and the Aged Receivables **cannot** yield a collection profile (it ages by *due date* = how overdue today's balances are, not the payment history). **Biggest find: Xero exports do not contain their own totals** — every "Total" row is an uncalculated Excel formula that a parser reads as **zero**, so the intake must sum the line items itself. **Blocker CLEARED 2026-07-15:** a populated real-company export pair (Electric Bikes NZ Limited — balance sheet + current-FY-by-month P&L) verified the stock / debtor / creditor mappings. Key finding: stock can be **split across several differently-named rows** (this company: 4), so the file proposes candidate rows and the advisor confirms which count — the hybrid-intake decision validated on real data. Monthly sales are now FILE-seedable via the by-month export; prior-year revenue stays advisor-entered. Cached totals exist in real-org exports → use as a cross-check, still never as the source. See [`design/REPORT-DATA-MODEL.md`](REPORT-DATA-MODEL.md) §3.9. **Prior-year revenue RULED 2026-07-15 (hybrid: file if supplied, manual always)** — see the decision log. **ALL remaining §6 owner questions RESOLVED 2026-07-16** (decision session + a real Aged Receivables Detail export + owner's live Xero check): collection profile can **never** be file-sourced (Xero never records money-received dates) → use debtor days + ageing buckets, per the new **models-adapt-to-the-exports** principle; `initialInvestment` + owner's drawings = hybrid; no-stock service businesses = adapt (stock leg drops out). **The data-model design is decision-complete** |
...
| T13 | Design the **de-identification (scrubbing) step** — strip client names, ID numbers, bank account numbers, and any identifiers from dropped files before the AI step | Design | P1 | ◐ | **RE-SCOPED by T25 — WIDER than the title suggests. The trigger is REAL CLIENT NUMBERS, not a dropped file.** So this covers **Decision tools too** (5 models), which import no file at all but take the client's real loan balances / property values / retirement position by keyboard — every bit as sensitive. Education models (5) are the only exempt class. The boundary that makes the privacy promise true; runs backend-only. **Boundary drafted 2026-07-13 → [`design/REPORT-DATA-MODEL.md`](REPORT-DATA-MODEL.md) §5** — held-locally vs. sent-to-AI table, incl. two easily-missed leaks: **(a) the uploaded file's own metadata** (filename, author, sheet names routinely carry the client's name) and **(b) the Aged Receivables export lists individual customers by name** — our richest source *and* our most identifying, so it must be reduced to ageing-bucket totals **on ingest**, before storage, not merely before the AI call |
...
| T10 | Design the **drag-and-drop file intake** — which reports, which file formats, and how they're read | Design | P1 | ◐ | **RE-SCOPED by T25: Report class ONLY (9 models, none built). Education models import nothing; Decision tools take real numbers typed in, not from a file — so neither needs intake.** Core UX; depends on T4 answer. **Contract drafted 2026-07-13 → [`design/REPORT-DATA-MODEL.md`](REPORT-DATA-MODEL.md) §4.** Owner decisions: **hybrid** (file seeds, advisor completes, every figure tagged *from file* / *entered*) and **spreadsheets only — XLSX + CSV, PDF rejected** (a PDF has no cells; figures inferred from layout can be silently misread, and a wrong number in front of a client is worse than an extra step). Parsing runs on the **Restify backend**, not Nuxt. Never fabricate a missing figure — ask the advisor. ~~Blocked on: a sample Xero export~~ **UNBLOCKED:** real exports supplied 2026-07-13 (Demo Company) + 2026-07-15 (populated real company) — file shapes verified, see [`REPORT-DATA-MODEL.md`](REPORT-DATA-MODEL.md) §3/§3.9 |
...
| B1 | **Fix garbled characters in report/launcher text** — mojibake (`�`) appears where em-dashes / bullets should be; an encoding bug in the mockup text | Bug | P2 | ☑ | Owner-reported 2026-07-09 (EOD). **Fixed 2026-07-13.** Root cause was *not* corrupted text: the four mockups had no `<head>` at all (each began at `<style>`), so with no `<meta charset>` the browser fell back to Windows-1252 and mis-rendered valid UTF-8 bytes (em-dash verified on disk as `e2 80 94`). No glyphs replaced — all four now open with `<!DOCTYPE html>` / `<html lang="en">` / `<head><meta charset="utf-8">` + title |
| B2 | **Add "Back to navigation" on every model page** — after opening a model from the launcher there's no way back to the Model Library | Bug | P2 | ☑ | Owner-reported 2026-07-09 (EOD). **Closed 2026-07-13.** Mockups: "← Back to Model Library" link added to all 3 model mockups. App: the link had nowhere to go — the launcher existed only as a mockup — so the owner chose to build the library for real (see T24). All 3 report components now carry a `<nuxt-link>` back to `/model-library`, label via `$t('modelLibrary.backToLibrary')`. Verified in the running app: the link renders and resolves on all three report routes |
| T26 | **Model Library — group the grid BY CLASS, with category filtering underneath** | Build | P2 | ☑ | Owner-requested 2026-07-13; **built + owner-verified live 2026-07-15** (commits `2cc2408`…`060273e`). The library now opens on **one choice — a type row: All · Teaching tools · Decision tools · Client reports** — with the grid grouped into those three titled shelves (heading + one-line description + count) under "All". **Owner ruling 2026-07-15 (drill-down):** the category row (Cash Flow / Profitability / …) **only appears after a type is chosen**, and resets to All on a type change so a topic picked earlier can never keep filtering invisibly. Search + type + topic all compose. Logic in [`utils/reportModelCatalogue.js`](../utils/reportModelCatalogue.js) (`CLASS_ORDER`, `groupByClass`, class-aware `filterModels` — 42 unit tests; an unknown-class model surfaces on a visible "other" shelf, never silently dropped); presentation in [`components/ModelLibrary.vue`](../components/ModelLibrary.vue); shelf/chip strings in `locales/en.json` (owner-approved wording, English with the usual 7-locale fallback) |
| T25 | **Classify every model — Education vs Decision tool vs Report** | Design | P1 | ☑ | **Owner-settled 2026-07-13 → [`design/MODEL-CLASSIFICATION.md`](MODEL-CLASSIFICATION.md).** The owner stopped a build heading the wrong way: the 3 built models are **not reports** — they are **teaching tools** (illustrative numbers whose job is to make a concept land). Evidence agreed: ~⅔ of their inputs don't exist in any accounting export. **All 19 catalogued models now classified: 5 Education, 5 Decision tool, 9 Report.** Each class has different needs — file intake, privacy, the *Illustrative* badge, and the accuracy bar all follow from the class. **Two corrections this produced:** (a) the 3 built models need **no file intake and no scrubbing** at all; (b) **privacy is triggered by real client numbers, not by a file upload** — so Decision tools, which import nothing and are typed in by hand, still handle sensitive data and still need T13. **Nothing built needs to change** |
| T24 | **Model Library page (`/model-library`)** — the launcher mockup built as a real Nuxt 2 route: 19-model catalogue, category filters, free-text search, live "X of Y · N ready" count | Build | P2 | ☑ | Built 2026-07-13 (owner-approved) to give B2 a destination and stop the reports being dead ends. Files: [`utils/reportModelCatalogue.js`](../utils/reportModelCatalogue.js) (catalogue + pure filter logic, 22 unit tests), [`components/ModelLibrary.vue`](../components/ModelLibrary.vue) (Pug + Options API + Buefy input), [`pages/model-library.vue`](../pages/model-library.vue), `modelLibrary.*` strings in `locales/en.json` (other 7 locales fall back to `en`). Only the 3 built models are clickable; the other 16 render inert, so nobody can click into a page that doesn't exist. **Deliberately excluded: the mockup's client-situation coach** — that is T22/T23 design work and its matching logic belongs on the Restify backend, not in a Nuxt component. Model names/summaries are English-only data for now (owner decision) and move server-side under T22 |
...
| 2026-07-15 | **Prior-year revenue (#12) = hybrid (owner).** Seeded from a prior-year P&L export **when the advisor supplies one**; **manual entry always available**; and if the figure isn't in what they dropped, the likeliest reason is the client doesn't have the data — offer the field, don't chase the file. Recorded in [`REPORT-DATA-MODEL.md`](REPORT-DATA-MODEL.md) §3.3 / §6. With this, the file-intake design has no open file-related questions. |
| 2026-07-16 | **Design principle (owner): models adapt to the exports, not the exports to the models.** Report-class mathematics is built on figures that standard accounting software easily produces (P&L, Balance Sheet, by-month P&L, aged receivables/payables) plus advisor-entered operational facts — never on data shapes that would force accountants into workarounds. Prompted by the collection-profile finding (next row). Education models are untouched (illustrative numbers, no intake). Guard: each Report-class model's adapted maths is shown to the owner at its design time — the standing "never simplify or remove any element of a source model without asking first" rule (2026-07-09) holds. |
| 2026-07-16 | **Collection profile can NEVER be file-sourced — owner-verified in Xero.** A real Aged Receivables **Detail** export was supplied and read: invoice-level (invoice date + due date per row) but only *unpaid* invoices, **no paid-date column**; the owner confirmed in the live product that Xero ages only by due date or invoice date — money-received dates are never in these reports. Report-class models use **debtor days** + **ageing buckets** (both file-available) or advisor entry instead. Privacy confirmed: the Detail export names the client and its customers on every row → reduce to bucket totals on ingest (T13/§5). [`REPORT-DATA-MODEL.md`](REPORT-DATA-MODEL.md) §6. |
| 2026-07-16 | **`initialInvestment` (#1) AND owner's drawings (#19) = hybrid (owner).** Both follow the prior-year-revenue pattern: seeded from the dropped file when identifiable, advisor confirms or overrides, tagged *from file* / *entered*. Drawings keep their Margin-model meaning as a target to cover — the seed starts that conversation. |
| 2026-07-16 | **No-stock service businesses = ADAPT (owner).** A Report-class model omits the stock inputs and tiles entirely for a service business and calculates the rest correctly — no zero boxes, no models withheld. With these four rulings **§6 of the data model has no open questions** — the Report-class data-model design is decision-complete. |
| 2026-07-16 | **First Report-class model = `CM. Quick Position.` (owner's pick from the 9).** Chosen as the most bounded model that runs on exactly the export figures verified real (Balance Sheet: cash/debtors/stock/creditors; P&L: the expense lines feed its Expenses Review table) — so it exercises the FULL new pipeline (file drop → parse → hybrid confirm → scrub → calc → report screen) as the Report-class walking skeleton. Bonus: "Quick Position" is already a crisis tool the AI coach recommends — this gives it a live destination. |
| 2026-07-16 | **Quick Position scope ruling (owner): the NZ COVID wage-subsidy block is DROPPED from the app version.** The source sheet ("Lockdown Diagnosis") hard-codes 2020 NZ policy ($585/$350 per week × 12 weeks); it and its two dependent cells (Subsidy Min, Sales Required After Subsidy Period) do not port. Everything else ports faithfully — no other maths adaptation needed. Ruled under the standing "never simplify or remove any element of a source model without asking first" rule. |
| 2026-07-16 | **Quick Position correctness proof PASSED — 32/32 golden cells.** Independent re-implementation reproduces every in-scope calculated cell exactly (Quick Cash Available, expense cycles at zero sales, life-line trading cycles, break-even sales required, the full sales-discount block, and the Expenses Review totals). These 32 values are the golden reference for the future backend engine's tests — same standard as the WCC's 28. |
| 2026-07-16 | **Quick Position BUILD PLAN approved (owner) — 4 approval-gated stages.** A = calc engine + route (golden-tested); B = the file-intake parser (Balance Sheet + P&L, sum-line-items rule, metadata stripped, parse-and-discard, `firmAuth` on the upload route); C = the 3-step screen per the approved mockup, i18n-compliant from day one (`report.quickPosition.*`); D = wire-up + live click-through with a real export. v1 sends nothing to AI (templated Coach) and persists nothing — parsed figures live only in the advisor's session. |
| 2026-07-16 | ~~**New dependency RULED (owner): `xlsx` (SheetJS) pinned `0.18.5`**~~ **SUPERSEDED the same day — see the next ruling.** The pre-install security review found `xlsx@0.18.5` (the newest npm version) carries **CVE-2023-30533** (HIGH 7.8, prototype pollution *when reading crafted files* — exactly the intake's use) with **no fixed version on npm** (vendor left the registry; fixes ship only from their own CDN), plus a second known HIGH (ReDoS, fixed only ≥0.20.2). Separately: this laptop has only npm 6, which ignores the reconciliation's `overrides` and would rewrite the npm-8 lockfile — **no npm install is safe on this machine** until it gets the desktop's npm-8 + Avast-cert setup. Nothing was installed. |
| 2026-07-16 | **XLSX reading RULED (owner): in-house zero-dependency reader** — `server/report/intake/xlsxReader.js`, built on Node's own zlib (an .xlsx is a ZIP of XML). Reads only what the intake needs (cell values); hardened for hostile files: bounds-checked offsets, inflate caps (zip-bomb guard), part/cell count caps, prototype-less maps. No dependency, no lockfile change, no CVE inheritance. CSV likewise dependency-free (`csvReader.js`). |
| 2026-07-16 | **Stage B BUILT — file-intake parser live on the backend.** `xeroReportParser.js`: recognises Xero Balance Sheet / P&L by their own title rows; **sums line items, never trusts a "Total" row** (a poisoned cached total is out-voted AND warned about); stock/wages surface as multi-row candidates (the §3.9 finding); wages/PAYE scoped to the liabilities side; PDF refused by name with guidance; unknown files fail loudly, no partial parse. Route `POST /api/report/quick-position/intake` **with `firmAuth`**, 5 MB cap, parse-and-discard, error-code-only logging (no filenames/labels/content ever logged). 38 new tests incl. the hostile-file battery (zip bomb, lying sizes, `__proto__`, binary junk); suite 1,139 green; lint 0 errors; intake code at 100% lines/functions. Remaining Stage B proof: the real-export click-through happens at Stage D. |
| 2026-07-16 | **Stage A BUILT — Quick Position calc engine live on the backend.** `server/report/quickPositionModel.js` (+ Expenses Review maths) + `POST /api/report/quick-position` registered beside the four existing model routes. 22 unit tests: all 32 golden cells, the no-stock exclusion, honesty guards (div-by-zero → `null`, never a fabricated 0; insolvent-now reads as a truthfully negative position), junk-input robustness. Full suite 1,101 green on Node 14.15; lint 0 errors. |
| 2026-07-16 | **Stage C BUILT — the Quick Position screens live at `/quick-position`.** `pages/quick-position.vue` (stepper + advisor-page token resolution) · `components/QuickPositionIntake.vue` (drag-and-drop + chooser, authenticated upload, confirm table with *from file*/*entered* tags, tick-box stock candidates, dates-agree check, cross-check warnings, no-stock toggle, skip-to-manual) · `components/QuickPositionReport.vue` (hero band, runway bar with the accepted red&lt;3/amber&lt;6 thresholds, discount-trap card, P&L-seeded expenses review + one-click use-as-fixed-costs, templated Coach, privacy strip, print, reset; every recalc is a debounced backend call). Model Library card now live (5 ready) wearing the *Client report* class — never Illustrative. **i18n-compliant from day one** (`report.quickPosition.*`, ~90 keys; 7 locales fall back) — the new screens do NOT repeat the logged hardcoded-English defect. Verified: lint clean, suite 1,139 green, `nuxt build` green (`pages/quick-position.js` emitted). Honest gaps: no component-level tests (repo-wide infrastructure gap, logged) — screens proven by the Stage D click-through; a few small labels beyond the mockup await the owner's wording review at Stage D. |
| 2026-07-17 | **Stage D DONE — Quick Position COMPLETE (A–D), live click-through PASSED.** The owner walked the full pipeline at `/quick-position` on this laptop with a real Xero export — drop → confirm → report — and accepted the screens, numbers and wording as shown ("looks great"). Stage D earned its keep at boot: **one wire-up defect caught + fixed** — `quickPositionIntake` was `async` *and* declared the `(req, res, next)` callback signature, which Restify 9's mount-time assertion refuses, so the backend would not start. Fixed to the async `(req, res)` convention (commit `4ae9b57`); auth, error codes and parse-and-discard unchanged. Why 1,139 tests missed it: route handlers are unit-tested by direct call, never mounted on a real Restify server — mount-time checks only run live (known gap, same family as the logged component-test gap). Env note: laptop nvm4w carries Node **20.20.2** for the Nuxt dev server alongside the locked 14.15 backend runtime — the Stage D open question from 2026-07-16 is resolved. Quick Position is the first Report-class model to complete the full new pipeline: file drop → parse → hybrid confirm → scrub → calc → report screen. |
| 2026-07-16 | **Quick Position screen design APPROVED by owner** — interactive mockup at `design/mockups/quick-position-mockup.html`, clicked through live. Three-step flow: **(1) Drop the exports** (Balance Sheet required, P&L optional; XLSX/CSV only, PDF refused with guidance; skip-to-manual path), **(2) Confirm the figures** — the FIRST hybrid-intake screen: every figure tagged *from file* / *entered*, multi-row stock candidates with tick-boxes (the Electric Bikes finding), reporting-dates-agree check, and the "service business — no stock" toggle (the adapt ruling, demonstrated), **(3) the report** — hero band (Quick Cash Available · Cover at Zero Sales · With Life-line · Break-even Sales), survival-runway bar, discount-trap card, P&L-seeded expenses review with one-click "use as monthly fixed costs", templated Coach box, on-screen privacy strip. Draft wording and the red&lt;3-months / amber&lt;6-months runway thresholds accepted as shown (owner may revisit at build time). Coach text templated for first build, per the WCC precedent. |
| 2026-07-17 | **Second Report-class model = `EBITDA Model` (owner's pick) — deconstructed; owner rulings recorded.** Deconstruction preview saved at `design/mockups/ebitda-dcf-deconstruction-preview.html` (source-sample figures). Rulings: **(1) reading CONFIRMED** — 5-year P&L Review normalising to EBITDA (owner pay out, fair-market package in) feeding a 5-year grow-then-discount DCF + terminal value → Enterprise Value; **(2) the two `#REF!`-broken remnants are DROPPED** — the entire Interest Calcs sheet (needs the deleted 'Purchase Data' sheet; loan-schedule module, nothing live uses it) and the Margin-of-Error/Capitalised-Earnings side workings (need the deleted 'Assumptions' sheet); owner confirmed no complete master workbook exists to recover. Ruled under the standing "never simplify or remove without asking" rule, per the Quick Position COVID-block precedent; **(3) the listed-company block is KEPT** — assessed-vs-market share price on the same DCF engine. Faithful-port notes: the source discounts each future year once at that year's rate (its own convention, not textbook compounding) — ported exactly; the 2026 projection falls back to 2024 when the 2025 EBITDA is blank (a corrected read — an earlier "self-referencing cell" description was an extraction artefact, fixed in the preview). |
| 2026-07-17 | **EBITDA & DCF correctness proof PASSED — 96/96 golden cells.** Independent re-implementation from raw inputs only reproduces every in-scope calculated cell exactly: all 10 calculated P&L-Review rows × 5 years (Gross Profit/GP%, Net Operating Profit/OpEx%, sundry subtotal, NPBT, add-back subtotal, EBPITDA, owner-benefits subtotal, normalised EBITDA), the period counter, and both DCF blocks end-to-end (actual/average growth, projected + discounted years, sum, terminal, **Enterprise Value 4,420,963**; market cap, **assessed share price 0.9258** vs 0.59). These 96 values are the golden reference for the backend engine's tests — same standard as WCC's 28 and Quick Position's 32. |
| 2026-07-17 | **EBITDA & DCF screen design APPROVED by owner** — interactive mockup at `design/mockups/ebitda-dcf-mockup.html`, clicked through live. Three-step flow per the Quick Position pattern: **(1) Drop up to five years of P&L exports** (multi-file, one per year or comparative; adapts down to 2 years; XLSX/CSV only; skip-to-manual), **(2) Confirm & normalise** — hybrid confirm with *from file* / *entered* tags; full add-back line set in the build (condensed in mockup); the source's "Accountant's internal comments" column becomes an optional session-only advisor note per line, **(3) the valuation report** — hero band (Enterprise Value · Normalised EBITDA · Discounted 5-yr CF · Terminal Value with editable exit multiple), actual-vs-projected earnings bars, condensed P&L review (expandable), live valuation workings (per-year growth + discount, debounced backend recalc), listed-company lens as its own card (behind a tab in the build), templated Coach, privacy strip, print, light/dark. Draft wording accepted as shown (owner may revisit at build); Coach templated, no AI in v1. |
| 2026-07-17 | **EBITDA & DCF Stage A BUILT — calc engine live on the backend.** `server/report/ebitdaDcfModel.js` (P&L normalisation + both DCF blocks, all per-year arrays oldest-first) + `POST /api/report/ebitda-dcf` beside the five existing model routes. 28 unit tests: **all 96 golden cells**, adaptive periods (runs on 2–5 years of accounts), honesty guards (zero-prior-year growth → `null` and excluded from the average; zero shares → `null` price; insolvent-now values truthfully negative; the sheet's own zero-sales guard ports as its written 0), junk-input robustness. **Plus the Stage-D lesson institutionalised:** `tests/unit/reportRoutes.mount.test.js` mounts every report handler on a real Restify server inside the suite, so a Restify 9 handler-signature boot failure can never again hide from `npm test`. Full suite 1,167 green on Node 14.15; lint 0 errors; route live-probed (Enterprise Value 4,420,962.96 · EBITDA 571,795 · assessed price 0.9258 — golden exactly). |
| 2026-07-17 | **EBITDA & DCF Stage B BUILT — multi-year P&L intake live on the backend.** Parser now yields the model's 8 figure buckets per P&L (sales/costOfSales/opex/otherIncome/interestReceived/dividends/badDebtsRecovered/loanInterestPaid — each row lands in exactly ONE bucket, no double-count) + the report's own year. New pure `annualAssembler.js` enforces the 1–5-file rules loudly: Balance Sheet in the mix → whole request rejected naming the position; duplicate/unreadable years → assembly blocked, screen resolves; cross-company names → warning; year gaps → warning. Route `POST /api/report/ebitda-dcf/intake` with `firmAuth`, 5 MB/file, parse-and-discard, error-code-only logging. **Latent QP parser bug found + fixed at the root:** Xero opens P&L sections as "Less Cost of Sales" but closes as "Total Cost of Sales" — the close never matched the open, nesting sections wrongly (harmless in QP's simple grid, wrong on richer P&Ls); `sectionName()` normalises the Less/Plus prefix, QP suite proves no regression. **v1 scope note:** one file per year; the comparative-export (one file, many years) variant awaits a REAL Xero comparative P&L sample from the owner before its parser is built — per the "models adapt to the exports" principle. 19 new tests; suite 1,186 green; intake code 100% lines/functions; unauthenticated intake probe → 401. Real-export click-through = Stage D. |
| 2026-07-17 | **EBITDA & DCF Stage C BUILT — the valuation screens live at `/ebitda-dcf`.** `pages/ebitda-dcf.vue` (stepper + advisor-page token resolution) · `components/EbitdaDcfIntake.vue` (multi-file drop, up to 5 P&Ls in one authenticated request; a year-assignment step appears only when a file's own date line carried no year; full confirm-&-normalise table — every source-model line × every year, *from file*/*entered* tags, editing a file cell re-tags it as the advisor's; skip-to-manual) · `components/EbitdaDcfReport.vue` (hero band with editable exit multiple, actual-vs-projected earnings bars with honest red negatives, expandable 5-year P&L review, live valuation workings — every edit a debounced backend call, listed-company lens behind its own toggle with the Undervalued/Overvalued reading, templated Coach incl. dip-year + terminal-sensitivity sentences, privacy strip, print, reset). ~120 i18n keys `report.ebitdaDcf.*` (7 locales fall back). Model Library card live — **6 ready** — *Client report* class; the 3 catalogue integrity tests consciously updated 5→6. Verified: lint 0 errors, suite 1,186 green, `nuxt build` green (`ebitda-dcf` bundle emitted), dev server serves the page + library card links. Honest gaps: no component tests (logged repo-wide gap) — screens proven at Stage D; small labels beyond the mockup await the owner's wording review at Stage D. |
| 2026-07-17 | **EBITDA & DCF Stage D DONE — model COMPLETE (A–D), live click-through PASSED ("all good").** The owner walked the full pipeline at `/ebitda-dcf` with real P&L exports — multi-file drop → confirm & normalise → valuation — and accepted screens, numbers and wording as shown. No defects surfaced (the mount boot-check from Stage A had already retired the class of wire-up failure that bit Quick Position). The second Report-class model is live end-to-end; the Report pipeline (drop → parse → hybrid confirm → calc → report) has now carried two models. Same day: owner ruled the **merge plan** — bring the desktop's 6 newest course-builder commits (incl. the languageName-injection security fix) into this branch, full gate, then one merge to `master`. |
