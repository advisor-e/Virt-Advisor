...
- [x] T022a [US1] Industry field on the intake offers suggestions from the engine's industry vocabulary as the advisor types *(built 2026-09-11 from drawing 4, `design/mockups/outcome-learning-intake-industry.html`, approved the same day: the intake has no industry field — the engine asks it in the chat — so the suggestions are chips under the chat box while that question is live; `questionDoneEvent` carries the field, `GET /api/advisor/industry-vocabulary` serves the words, `utils/industrySuggestions.js` filters them)* (Mike's ruling on drawing 2, 2026-09-10: *"start typing - it makes suggestions - if no match - doesnt save"*); the component is named at build from where the intake collects `industry`; the typed value still saves to the case, and `buildContribution` (T011) pools industry only on an exact vocabulary match
...
- [x] T030 [US2] Write `scripts/dev/seed-outcome-pool.js` — writes a seeded pool through `guardContribution` and `saveFirmConfig` (never around them) for quickstart Story 2; dev only, refuses to run when `NODE_ENV=production`. *(Built 2026-09-11: 31 reviews across five firms, Break-Even above the floor at hold-back 4 and 7 Cash Drivers below it; firm A is `dev-firm` so its own withdrawal takes its rows out; `--reset` clears the seeded firms' prefixes first. Needs `OUTCOME_POOL_SECRET` and the local MySQL; not yet run against it.)*
...
**Checkpoint**: quickstart Story 4 passes — SEEN RUNNING 2026-09-11 on the production build against local MySQL: "Run the benches" pressed, the four figures and the "Last run …" line appeared, a new saved version landed in history. On the seeded pool the outcome bench reads 0% both ways because the engine's own pick for every seeded situation is "8 Profit Levers", which the seed never reviews — the seed's data, not the bench.
