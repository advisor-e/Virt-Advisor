...
> 🔴 **T007–T018 WERE BUILT ON 2026-09-14 AND LEFT UNTICKED; AUDITED AGAINST THE CODE AND
> TICKED 2026-09-15.** Every one was verified in the source, not by the presence of a file:
> the three trace keys are written in `advisorEngine.js`, `buildContribution` reads them,
> the fixtures build `situation` as the string the engine really produces, the AI block and
> `aiProvider.js` exist with `AI_PERSONAL_FLAG_MISSING` and `logSuffix`, `SCORING_VERSION` is
> `2.3.0`, and `rankLabels` / `parseReply` / `proposalLine` are all present. 212 tests across
> their suites pass.
>
> **ONE DIFFERENCE, recorded rather than smoothed over: T014's assertions live in
> `tests/unit/resolverSeams.test.js`, not in a `resolveIndustryWord.test.js`** — that file does
> not exist. The substance is there (plural, case, stop word, commercial noise, and agreement
> with the resolver's own title match); only the filename differs.
>
> **Why this matters more than the ticks.** A session that finished the work and not the record
> left the next one reading "36 of 67" and about to rebuild a dozen finished tasks. On this day
> one nearly did: it told Mike every pooled row still had a blank primary issue — reading the
> list, not the code — when the fix had shipped the day before. **Tick the task in the same
> change as the code, and when a list and the code disagree, the code wins.**

- [x] T007 In `server/advisorEngine.js` add three top-level keys to `_decisionTrace` (data-model §1): `primaryIssue: { label: null, how: 'none', reason: null }` (filled by US1), `industry: state.industry || null` (the `'skipped'`/`'pending'` sentinels mapped to null), and `ai: { provider: 'openai' }` (filled by US8); leave the `situation` string untouched with a comment naming `resolveSavedClientContext` as its reader
- [x] T008 In `server/utils/outcomeLearning.js` `buildContribution`, read `trace.primaryIssue.label` and `trace.industry` instead of `trace.situation.*`; delete the object test on `situation`; JSDoc records the 2026-09-14 finding (the string situation made both fields null on every live row)
- [x] T009 Update `tests/unit/outcomeLearning.test.js` and `tests/unit/casesContribute.test.js` fixtures to build `situation` as the **string** the engine writes and the two new keys, so the tests can no longer pass on a shape the engine never produces
- [x] T010 [P] Add the `AI` block to `config/integration.js` (data-model §7): primary and fallback name/host/key/chat path, the role→model map for both, `fallbackPersonalCleared` (default false), every value from `process.env`; JSDoc names DeepSeek's China hosting and training terms as the reason the clearance defaults to false; add the variables with comments to `.env.example` and a section to `design/UAT-LOAD-PACK.md`
- [x] T011 [P] Create `server/utils/aiProvider.js` (contracts §Provider seam) — `getClient(role)` returning `{ chat: { completions: { create(params, options) } } }`: `options.personal` required (throw `AI_PERSONAL_FLAG_MISSING` if absent); primary via `createOpenAIClient({ apiKey, host })`; on a thrown error, 401/402/429/5xx, or an empty reply, one fallback attempt only when configured and (`!personal || fallbackPersonalCleared`), with the role's fallback model substituted; reply gains `provider`; a helper `logSuffix(reply)` → `provider=<name> fallback=<none|used|refused-personal>`; streaming passes through the same rule (the fallback is only tried before the first delta)
- [x] T012 Write `tests/unit/aiProvider.test.js` at **100%**: primary ok → no fallback call; primary throws / 401 / 402 / 429 / 500 / empty → fallback called once with the fallback model; personal + not cleared → fallback never called and the primary's error rethrown with `refused-personal` in the suffix; personal + cleared → fallback used; no fallback configured → behaviour identical to today; flag missing → throws
...
- [x] T014 [P] Write `tests/unit/resolveIndustryWord.test.js` — "cafes" → the vocabulary word the café model's title carries; a stop word → null; nothing matching → null; the same result as the resolver's own title match on a fixture
...
- [x] T039 [US5] In `server/routes/outcomeLearning.js` `list` add `reach` summed over firms whose consent is on (via `listFirmIdsWithConfigKey(CONFIG_KEY)` + `contributionOpen`), cached 60 s beside the live list; in `server/routes/outcomeConsent.js` `read` add the firm's own pair; update both route tests
- [x] T040 [US5] Render the reach tiles in `components/mentor/MentorOutcomeLearning.vue` and the pair in `components/firm/FirmOutcomeConsent.vue` per the approved drawing; `locales/en.json` keys
...
- [x] T052 [US8] Record the provider: `trace.ai.provider` **from the session's distinction-classify calls, NOT from the recommendation call as this task first said** — that call is a stream, `aiProvider._tag` skips an async iterable by construction, and the trace is sent inside that stream's own finish handler; the classify calls run earlier in the same request through the same seam and do carry it. `provider` beside `model` on the stored meeting reports and hub reading was already done under T051; transcription has no fallback by construction (T053). `components/VirtualAdvisor.vue` renders the *Answered by* line per the approved trace drawing, absent where nothing answered rather than naming a service on a guess. Deviation recorded in `design/ARTEFACTS.md`
...
- [x] T056 [US9] Create `server/utils/semanticProfiles.js` — `PROFILE_PREFIX = 'semantic-profile:'`, `loadEffectiveProfiles()`, `isThin`, `validateProfile`, `clearProfileCache()`; indicators read from `content-summaries.json` via `templateRegistry`
- [x] T057 [US9] Create `server/routes/semanticProfiles.js` (contracts §New routes) and register the routes in `server/restify-server.js` under `mentorGuard`. **Done 2026-09-16 — ONE ROUTE OF THE CONTRACT'S FOUR**, per Mike's read-only ruling below: `GET /api/mentor/semantic-profiles` alone. `PUT /:page`, `GET /:page/history` and `POST /:page/restore` are deliberately NOT registered and go with T058. Verified by calling the handler: 200, **220 tools on 205 pages, 55 thin, 15 signals, 14 pages carrying more than one tool**. The handler is synchronous (everything it reads is `require`d at load) and holds the store as a module rather than destructuring it, so the refusal path can be proved
- [x] **T058 BUILT 2026-09-16 ON MIKE'S SECOND RULING OF THE DAY.** He was told plainly what it does — an authored profile overrides the compiled guess and changes what every advisor is recommended, on the dominant lever, with no test able to judge a weight — and said yes, because 44 tools had no profile at all. In `server/advisorEngine.js` load `loadEffectiveProfiles()` beside the pooled read and pass `options.profileMap` into both resolver passes; `server/utils/outcomeBench.js` and `scripts/scenario-lab.js` do the same so the benches score what the engine scores. **This is the one task in US9 that touches what advisors are recommended**, on the resolver's dominant lever, with no test able to judge whether a weight is right. It goes with the authoring half of T057 (PUT, history, restore) as one decision, after he has seen the screen and judged whether authoring 44 tools is worth his time
- [x] T059 [US9] Create `components/mentor/MentorSemanticProfiles.vue` from the approved drawing (Options API, Pug, Buefy, `$t('semanticProfiles.*')`), modelled on `MentorTemplateLibrary.vue`; register it in `components/FirmManagerHub.vue` with `TAB_TIERS.semanticProfiles = ['mentor']`, a `NAV_GROUPS` item appended to *Your AI coach*, a panel, and the tier judgement as a comment; add `semanticProfiles` to `MENTOR_ADDED_SINCE` in `tests/unit/hubTabTiers.test.js` with the ruling; `locales/en.json` namespace
...
- [ ] **T060 DROPPED ON MIKE'S INSTRUCTION, 2026-09-16** — *"make it work but save where you can"*, after he found 7.2 had consumed 75% of a week's token budget. The thin count is already on the screen and counted from the same rows, so a second copy in the lab report adds a number nobody reads twice. Re-open it only if the lab's output is being read for this. Original scope: `scripts/scenario-lab.js` prints "Templates with a thin effective profile: n/220" and marks each case's top card with whether a `semantic:` reason carried it; `scripts/build-semantic-profiles.js` header comment says authored rows live in the store and this file is the seed
...
- [x] T061 [US10] In `design/features/advisory-engine.md` rewrite §3's "designed, not built" paragraph, the §4 pipeline table rows 2–3, and the Known gaps line: primary-issue confirmation built by 4.97; routing groups removed by the registry's ruling of 2026-06-09, cited; in `design/virt-advisor-registry.md:551` correct the `__none_of_these__` sentinel line (handler deleted 2026-08-15). **Done 2026-09-17, each claim verified against the source first rather than taken from this task's wording:** `primaryIssueProposer.js` exists and is required at `advisorEngine.js:45`, so §3 no longer lists confirmation as unbuilt; the registry's own §375/§886 say routing groups are *"dead and removed"* with Stage 3 deleted, so the pipeline row and the Known-gaps line now say deleted rather than pending. **The sentinel line needed more than a correction:** `grep` found no `noneOfTheseApply` and no `__none_of_these__` anywhere in the code, but the *escape* survives in `_CONTRADICTION_PATTERN` one line above it — so the line now records that the mechanism changed from a menu option to plain language while the principle stands, instead of implying the escape was lost

---

## Phase 13: Polish and the record

- [x] T062 Open each approved drawing beside the built screen and name every difference in `design/ARTEFACTS.md` (deliberate deviations recorded; none unrecorded). **Done 2026-09-16 for `template-profiles.html` — four differences named on its row:** no row editor (the read-only ruling), 205 rows not 220 (the shared-page ruling), live tile figures not the drawing's (thin 61 → 55 after the recompile), and a search box the drawing lacks
- [x] T063 Run `npm run lint`, `npm test` with coverage, `npm run build`, `node scripts/scenario-lab.js`, `node scripts/outcome-bench.js`; record the METRICS block in `design/SCENARIO-LAB-REPORT.md`. **Done 2026-09-16 for the three that gate a release: lint 0 errors (456 warnings, unchanged — the new screen added none), `npm test` 531 suites / 11,264 tests green with coverage thresholds met, and `npm run build` SUCCEEDED ("Ready to run nuxt start")** — the last being the gate the master team hits at UAT and the one nothing else in this repo checks. **The two lab runs were NOT re-run, on Mike's instruction of 2026-09-16** (*"save where you can"*): nothing in this session touched the resolver, the engine or the benches — the route and screen are read-only and the engine still reads the compiled file — so neither report's numbers can have moved. ⚠ Re-run both before tagging if T058 is ever built, because that one does change scoring. Item 9.2 also stands open against the lab overwriting a full report with a partial one
- [ ] **T064 NOT DONE — deliberately deferred, Mike's yes 2026-09-17.** `quickstart.md` is written and unrun. Walking it needs the production build, local MySQL, an OpenAI key with credit and a second provider's key, to re-prove ten stories each verified as it shipped — against the *"save where you can"* instruction of 2026-09-16, after 7.2 took 75% of a week's token budget. ⚠ **Re-open it if the engine's scoring is touched again**: 7.2's own record is that five faults came from driving the running app and none from the suite, so a walk earns its cost exactly when scoring moves
- [x] T065 Update `design/features/outcome-learning.md` §8 (lift, cap, reach, time split, the trace fault and its fix) and `design/features/advisory-engine.md` §4 (the proposer, `SCORING_VERSION` 2.3.0, `aiProvider`, the profile store); keep the prompt Mike approved verbatim in the brief; one fact, one home. **Done 2026-09-17. Both Briefs stated `SCORING_VERSION` 2.2.0 against the code's 2.3.0 (`templateResolver.js:48`) and both are corrected.** 🔴 **The load-bearing one was not a version number:** `outcome-learning.md:238` told a reader an adjustment *"can only hold a template back, never lift one, in this release"* — untrue since US2 made it signed, on the exact mechanism that decides what an advisor is shown. Mike's 2026-09-10 ruling is kept as the ruling it was, with US2 recorded as what superseded it, so the history is not erased. Rewritten to name the cap, the floor and the advisor's-words protection as applying to lifts identically. The `aiProvider` seam, the proposer and the profile store were **already documented correctly** and were left alone — one fact, one home
- [x] T066 Close 4.97 on `design/features/to-do-items.json` per the shutdown checklist: `activeOn` cleared, the note replaced, the closure written on `to-do-done-and-parked.md`. **Done 2026-09-17.** The item is 7.2 (renumbered from 4.97 on the 2026-09-16 merge); its whole block is removed from the live list — fifteen items to fourteen, `activeOn` gone with it — and its closure is written in §2 of `to-do-done-and-parked.md`, recording the nine stories, the five faults found by running the app and none by the suite, the IMPACT TEST measurement that came out of US9, and which of T060/T064 were dropped and why
