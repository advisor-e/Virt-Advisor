...
**Why this priority**: The profiles are the dominant scoring input. **The screen's subject is the 220 client-facing tools the engine scores**, not the 199 entries in the profile file and not all 291 templates in the library (Mike's ruling 2026-09-14, after he caught the first drawing counting the file rather than the library). Against those 220: **61 are thin** — 7 with no profile entry at all, 51 with an entry but no signals matched, 3 with weights summing below 4 — 149 were generated automatically and never reviewed, and 1 has been reviewed by a person. 170 of the 220 have a summary written about them. (Counts re-derived from the data files on 2026-09-14; the audit's earlier 23/88 figures, and this spec's own first draft of 199/46, are withdrawn.) They are Mike's content, and today nothing on any screen shows or edits them.
...
- **FR-016**: The Scenario Lab MUST report how many cases proposed a primary issue and how many of the 220 client-facing tools remain thin.
- **FR-014b**: Thin means one of four states, each named on screen with its reason: no profile entry at all, an entry with no signals matched, weights summing below 4, or a keyword-only match. The screen lists the 220 client-facing tools drawn from the library, never the profile file alone (clarified 2026-09-14).
